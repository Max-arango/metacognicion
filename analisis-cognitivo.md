...
- **No transfiere el juicio, transfiere la estructura.** Saber *cuándo* una sorpresa es
  ...
  transfiere.
- **La auto-crítica sin señal externa degrada lo fácil.** La evidencia empírica reciente
  (Self-Critique Paradox, 2025-26) muestra que forzar bucles de crítica/refinado en
  tareas donde el modelo ya es sólido reduce el acierto: el crítico inventa defectos
  donde no los hay. Por eso la regla de trivial (solo la línea OBJETIVO de R1) no es
  economía: protege el acierto. Y por eso la verificación de R5 en código se contrasta
  con ground truth externo (tests, linters, compilador), no con la auto-evaluación.

---

## 14. El bucle de mejora (de los datos al protocolo)

La instrumentación de la SKILL.md cierra un bucle que este documento no tenía: los datos
producen decisiones sobre el propio protocolo. El ciclo, por sesión:

1. **Emisión** determinista del bloque META-PROTOCOLO (fin de tarea, auditoría, corte de
   sesión) — no "al acordarme".
2. **Persistencia** en `estado/meta-protocolo-log.md`, una línea parseable por sesión
   (schema en `estado/README.md`).
3. **Umbrales**: cada métrica tiene objetivo y señal de problema (tabla en SKILL.md).
4. **Decisión**: N ≥ 3 sesiones con la misma señal ⇒ el problema es del protocolo
   (disparador, plantilla, costo de tokens), no de la aplicación — se ajusta el
   protocolo y se anota en el CHANGELOG con la evidencia.
5. **Memoria**: `estado/sesion-anterior.md` lleva restricciones vivas e incógnitas
   abiertas entre sesiones, porque el contexto no sobrevive al cierre.

El principio es el mismo que rige todo el documento: lo que no se escribe no existe, y lo
que no se mide no se mejora. La diferencia ahora: el que mide es un script y un umbral,
no el modelo que se está midiendo a sí mismo.
